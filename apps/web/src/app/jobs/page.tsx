...
export default function JobAdminPage() {
  ...
  const [performingJobId, setPerformingJobId] = useState<string | null>(null)
  ...
  const handlePerform = async (jobId: string) => {
    try {
      setPerformingJobId(jobId)
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/job/${jobId}/perform`, {
        method: 'GET',
        credentials: 'include',
      })

      if (!response.ok) {
        throw new Error('Failed to perform job')
      }

      toast.success('Job executed successfully')
      fetchSchedulers()
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to perform job')
      console.error('Error performing job:', error)
    } finally {
      setPerformingJobId(null)
    }
  }
  ...
  return (
    <div className="container mx-auto py-8 px-4 space-y-8">
      ...
      <JobsList
        ...
        performingJobId={performingJobId}
      />
    </div>
  )
}
